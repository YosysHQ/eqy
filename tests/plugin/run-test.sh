...
EQY_BASEDIR="$(cd "$(dirname "${BASH_SOURCE[0]}")"/../../ >/dev/null 2>&1 && pwd)"
...
generate_ys_test() {
	ys_file=$1
	yosys_args=${2:-}
	generate_target "$ys_file" "yosys -m $EQY_BASEDIR/src/eqy_combine.so -m $EQY_BASEDIR/src/eqy_partition.so -m $EQY_BASEDIR/src/eqy_recode.so -ql ${ys_file%.*}.log $yosys_args $ys_file"
}
